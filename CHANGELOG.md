...
- **vLLM as a remote provider**: vLLM is now a first-class, selectable provider. It is CUDA-only so it cannot run on Apple Silicon, but you can point it at a remote vLLM server (your NVIDIA box or a GPU host) and it works over the OpenAI-compatible transport - it lists that server's models and streams chat like any other endpoint. Selecting it opens an endpoint config for the server URL and an optional key.
